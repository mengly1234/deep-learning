...
def eformer_block(dim, index, layers,    # index 表示的是第几个stage
...
class EfficientFormerV2(nn.Module):
    def __init__(self, layers, embed_dims=None,  # layers = [2, 2, 6, 4] embed_dims = [32, 48, 96, 176]
        ...
        network = []
        for i in range(len(layers)):  # len(layers) = 4, 网络一共四层
        ...
        if self.fork_feat:
            ...
            self.head = nn.Linear(embed_dims[-1], num_classes) if num_classes > 0 \
                else nn.Identity()
    ...
    def forward(self, x):
        ...
        if self.dist:
            ...
            cls_out = self.head(x.flatten(2).mean(-1))   # linear 层
...
@register_model
def efficientformerv2_s0(pretrained=False, num_classes=1000, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_dict = torch.load('D:\pycharm\mobilenet\mobilenet\pretrained\eformer_s0_450.pth')
...
if __name__ == '__main__':
    model = efficientformerv2_s0(pretrained=False, num_classes=7)
