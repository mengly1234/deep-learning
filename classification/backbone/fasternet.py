...
def fasternet(num_classes = 15, pretrain = True):
    model = FasterNet(num_classes = num_classes)

    if pretrain:
        model_dict = model.state_dict()
        pretrained_dict = torch.load('../pretrained/fasternet_t0.pth')
...
if __name__ == '__main__':
    ...
    net = fasternet(num_classes = 15, pretrain=True).cuda()
    feat = torch.randn((1, 3, 224, 224)).cuda()

    out = net.forward_cls(feat)
    print(out.shape)