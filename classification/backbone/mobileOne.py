...
def mobileone(num_classes: int = 1000, inference_mode: bool = False,
    ...
    if not inference_mode:
        if variant == 's0':
            checkpoint = torch.load('D:\pycharm_project\mobilenet\pretrained\mobileOne\mobileone_s0_unfused.pth.tar')
        elif variant =='s1':
            checkpoint = torch.load('D:\pycharm_project\mobilenet\pretrained\mobileOne\mobileone_s1_unfused.pth.tar')
...
if __name__ == '__main__':
    ...
    print(device)
    img = torch.randn((1,3,640,448)).to(device)
    # 加载训练好的pt模型
    model = torch.load(r'D:\pycharm_project\mobilenet\run\exp_448_640_mobileone_s0_resize\best.pt')
    # D:\pycharm_project\mobilenet\run\exp_448_640_mobileone_s0_resize_nopretrain
    model = model.to(device)
    # 对模型进行重新参数化
    model_rep_pt = reparameterize_model(model)
    # 保存重新参数化之后的模型
    torch.save(model_rep_pt,r'D:\pycharm_project\mobilenet\run\exp_448_640_mobileone_s0_resize\best_rep.pt')
    # 加载重新参数化之后的模型
    # model_rep = torch.load(r'D:\pycharm_project\mobilenet\run\exp_640_mobileone_s0_add\best_rep.pt')
    # model_rep = model_rep.to(device)
    # # 加载mobilenetv2
    # model_mobilev2 = torch.load(r'D:\pycharm_project\mobilenet\run\exp_640_mobilenet_v2_add\best.pt')
    ...
    # t2 = time.time()
    # out1 = model(img)
    # print('rep前： ', time.time() - t2)
    #
    # t3 = time.time()
    # out_mobilev2 = model_mobilev2(img)
    # print('mobilev2: ', time.time()-t3)
    #
    # # 使用重新参数化之后的模型进行推理
    # t1 = time.time()
    # out_rep = model_rep(img)
    # print('rep后： ', (time.time()-t1))
