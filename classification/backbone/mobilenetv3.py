...
class MyHardswish(nn.Module):
    ...
    def forward(self, x):
        out = x * F.hardtanh(x + 3, 0., 6., inplace=self.inplace) / 6.
        return out
...
class MobileNetV3_Small(nn.Module):
    def __init__(self, num_classes=1000, act=MyHardswish):
...
def mobilenet_v3_small(pretrained=True, num_classes=1000):
    ...
    # print(model)
    if pretrained:
        pretrained_dict = torch.load('D:/pycharm/mobilenet/mobilenet/pretrained/mobilenet_v3/450_act3_mobilenetv3_small.pth')
        pretrained_dict.pop('linear4.weight')
        pretrained_dict.pop('linear4.bias')
...
def mobilenet_v3_large(pretrained=True, num_classes=1000):
    ...
    if pretrained:
        pretrained_dict = torch.load('D:/pycharm/mobilenet/mobilenet/pretrained/mobilenet_v3/300_act3_mobilenetv3_large.pth')
        pretrained_dict.pop('linear4.weight')
        pretrained_dict.pop('linear4.bias')
        model.load_state_dict(pretrained_dict, strict=False)
...
if __name__ == '__main__':
    t1 = time.time()
    feat = torch.rand(2, 3, 224, 224)
    model = mobilenet_v3_small(pretrained=False, num_classes=7)
    out = model(feat)
    ...
    t2 = time.time()
    model2 = mobilenet_v3_large(pretrained=False, num_classes=17)
    # print(model)
    out2 = model2(feat)
    print("mobilenet_v3_large")
    print(np.argmax(out2.detach().numpy(), axis=1))
    print(time.time()-t2, '\n')

    # t3 = time.time()
    # model3 = mobilenet_v2(pretrained=True, num_classes=17)
    # out3 = model3(feat)
    # print("mobilenet_v2")
    # print(np.argmax(out3.detach().numpy(), axis=1))
    # print(time.time()-t3, '\n')